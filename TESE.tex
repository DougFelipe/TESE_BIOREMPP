% =============================================================================
% TESE DE DOUTORADO — BioRemPP
% Bioremediation Potential Profiling: um ecossistema de ferramentas para
% análise funcional e integração multiômica orientada a compostos prioritários
% para biorremediação
%
% Universidade Federal do Rio Grande do Norte
% Programa de Pós-Graduação em Bioinformática
%
% Estrutura: Alternativa B (orientada a componentes) — 7 capítulos
% ETAPA 1: preâmbulo + pré-textuais + Capítulo 1 + Capítulo 2
% ETAPA 2 (a produzir após validação): Capítulos 3, 4, 5, 6, 7 + referências
% =============================================================================

\documentclass[12pt,a4paper,twoside,openright]{report}

% =============================================================================
% ESTRUTURA MODULAR
% =============================================================================

\input{preamble/setup}
\input{preamble/macros}
\input{preamble/acronyms}

\begin{document}

\input{frontmatter/cover}
\input{frontmatter/titlepage}
\input{frontmatter/catalog-card}
\input{frontmatter/approval}
\input{frontmatter/dedication}
\input{frontmatter/acknowledgements}
\input{frontmatter/epigraph}
\input{frontmatter/resumo}
\input{frontmatter/abstract}
\input{frontmatter/lists}

\input{chapters/chapter_01_introducao}
%\input{chapters/chapter_02_fundamentacao}
\input{chapters/chapter_03_comp1_database}
\input{chapters/chapter_04_comp2_dbx}
\input{chapters/chapter_05_comp3_bws}
%\input{chapters/chapter_06_comp4_cli}
\input{chapters/chapter_07_conclusao}

\input{backmatter/references}
\input{appendices/appendix_a_esquema_db}
\input{appendices/appendix_b_codigo_fonte}
\input{appendices/appendix_c_comprovantes}
\input{appendices/appendix_d_casos_uso_bws}

\end{document}
