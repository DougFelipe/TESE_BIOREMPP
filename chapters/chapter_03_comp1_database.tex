...
% -----------------------------------------------------------------------------
\section{Validação das relações funcionais com o KEGG}
...
A validação também examina o suporte das combinações
\texttt{(ko, ec, reaction)} nos registros com número EC e reação preenchidos.
O critério de suporte combinado, identificado nos relatórios como
\texttt{policy\_union}, aceita as combinações sustentadas por qualquer uma
das estratégias densas usadas na construção do banco. O critério
\texttt{strict5} exige concordância simultânea entre as cinco relações
consultadas e representa uma condição adicional de maior restrição. Seu
resultado não corresponde, isoladamente, à cobertura global da validação.
...
A validação analítica opera em dois modos. O modo de consistência interna
...
\subsection{Verificação do validador}

Testes automatizados verificam o comportamento do validador em execuções
aprovadas e em condições de falha conhecidas. Os cenários incluem arquivos
ausentes, alterações no esquema de dados, inconsistências nos metadados e
variações acima dos limites definidos. Os testes confirmam se cada condição
produz a resposta esperada e reduzem o risco de atribuir aos dados um erro
originado no próprio processo de validação.

A configuração completa dos critérios, a composição da versão de referência,
os cenários de teste e os procedimentos de execução estão documentados nos
repositórios oficiais da organização BioRemPP, disponíveis em
\url{https://github.com/BioRemPP}.
...
1.939 reações, 1.517 símbolos gênicos, 1.422 descrições funcionais e
206 descritores de atividade enzimática. Os 384 compostos foram associados a
pelo menos um dos nove códigos de \texttt{referenceAG}.

As predições do ToxCSM abrangeram 370 compostos, o que correspondeu a
96,4\% do inventário. Os 14 compostos restantes não apresentaram predições no
conjunto integrado.
...
\fbox{
    \parbox{0.75\textwidth}{
        \centering
        Gráfico de barras horizontais com as 12 classes químicas ordenadas
        pelo número de compostos distintos na versão 1.1.0.
        \vspace{2em}
    }
}
...
\fbox{
    \parbox{0.75\textwidth}{
        \centering
        Diagrama de sobreposição entre os nove códigos de
        \texttt{referenceAG}, com as interseções entre as fontes
        institucionais.
        \vspace{2em}
    }
}
...
O critério \texttt{policy\_union} sustentou as 120.432 linhas com número EC
e reação preenchidos. O critério \texttt{strict5}, que exigiu concordância
simultânea entre as cinco relações consultadas, sustentou 2,42\% dessas
linhas.
...
Suporte por \texttt{policy\_union}
& 100,0\% \\

Suporte por \texttt{strict5}
& 2,42\% \\
...
identificar a origem institucional de cada associação.

A composição das fontes também introduz assimetria geográfica e institucional.
...
organizadas por composto no ToxCSM.

Essa separação evita impor uma correspondência unívoca entre composto e grupo
...
associadas à entidade química e não são convertidas em propriedades gênicas.

A cobertura do ToxCSM alcançou 370 dos 384 compostos na versão 1.1.0. Os
...
A expansão relacional também mostrou diferenças entre a cobertura por KO e
por composto no KEGG. Na versão 1.1.0, 1.527 dos 1.543 KOs apresentaram
correspondência nas relações consultadas, enquanto apenas 169 dos
384 compostos apresentaram correspondência direta em nível de CPD. Os
215 compostos restantes permaneceram associados ao banco por relações
baseadas em KO ou por pares composto--KO incorporados durante a curadoria.
...
A ausência de uma relação nos links KEGG consultados não constitui evidência
...
corresponde ao número de compostos nem ao número de funções distintas.

A repetição de um identificador pode resultar de sua associação a diferentes
...
A aprovação das duas camadas na versão 1.1.0 indica que o arquivo atendeu aos
