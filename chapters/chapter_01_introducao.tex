...
\nota{Reformulados em relação ao draft anterior para refletir os quatro componentes na nova organização. Um objetivo por componente + objetivos transversais.}

\begin{enumerate}
    \item Construir uma base de dados curada e integrada que consolide compostos regulados por sete frameworks internacionais e regionais, mapeados sobre vias canônicas de metabolismo de xenobióticos do \gls{kegg}, complementadas por evidências de degradação especializada (\gls{hadeg}) e predições toxicológicas (\gls{toxcsm}), suportada por pipeline reprodutível em Snakemake com camada dupla de validação (integrada KEGG-anchored e release-level \gls{gx}).

    \item Desenvolver o \gls{dbx}, uma interface web \gls{spa} de acesso aberto para navegação read-only da base integrada, organizada em módulos compound-centric e complementada por recipes reproduzíveis de análise guiada.

    \item Implementar o \gls{bws}, um serviço web baseado em upload de perfis \gls{ko} do usuário, oferecendo módulos analíticos que operam contra a mesma base integrada, com métricas ecológico-funcionais e algoritmos de suporte à decisão para consórcios microbianos.
    ...
    \item Garantir a conformidade do ecossistema com os princípios \gls{fair} através do uso consistente de identificadores interoperáveis (\gls{ko}, \gls{cpd}, KEGG Reaction, \gls{ec}, \gls{chebi}, \gls{smiles}), arquivamento com \gls{doi} estável e licenciamento aberto (código Apache~2.0, dados CC~BY~4.0).

    \item Demonstrar a aplicabilidade do ecossistema retrospectivamente através de três estudos peer-reviewed do grupo que utilizaram a evidência integrada em contextos de caracterização de novas espécies, integração transcriptômica e metatranscriptômica, e desenvolvimento de consórcio industrial patenteado.
\end{enumerate}
