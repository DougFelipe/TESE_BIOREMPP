...
\begin{enumerate}
    \item Construir uma base de dados curada suportada por pipeline reprodutível em Snakemake que consolide compostos regulados por nove frameworks regulatórios, mapeados sobre todas as vias do \gls{kegg} e as 20 vias canônicas específicas do metabolismo e degradação de xenobióticos, integrada a bases externas complementadas por evidências de degradação especializada (\gls{hadeg}) e predições toxicológicas (\gls{toxcsm}).

    \item Desenvolver o \gls{dbx}, uma interface web de acesso aberto para navegação read-only da base integrada, organizada em entidades (Compostos, Classes, Genes, Vias e Toxicidade) e análise guiadas.

    \item Implementar o \gls{bws}, um serviço web baseado em upload de arquivos de anotação funcional de \gls{ko}, oferecendo módulos analíticos e casos de uso que operam utilizando a  base integrada, fornecendo uma camada de análsie de dados \textit{in sillico} para suporte a tomada de decisão e inferência.
    ...
    \item Demonstrar a aplicabilidade do ecossistema retrospectivamente através de três estudos peer-reviewed que utilizaram a base integrada em contextos de caracterização de novas espécies, subespécie, integração transcriptômica e metatranscriptômica
\end{enumerate}
