...
funcionais estabelecidas por CPD e KO.

A seleção de uma classe recupera os compostos apresentados sob o mesmo
...
derivadas do número de compostos e vias relacionados a cada KO.

A página de detalhe recupera os compostos, as vias, as atividades enzimáticas
...
Para cada via, a interface apresenta medidas derivadas das associações com
...
Cada resultado mantém a ligação com o CPD correspondente, o que permite
...
operação sobre a mesma versão do banco.}
...
Cada caso de uso possui uma receita SQL correspondente. Essas receitas
registram as tabelas consultadas, as junções, os critérios de seleção, as
...
módulos de exploração, a execução das análises guiadas e a verificação do
software. As cardinalidades e as propriedades científicas dos conjuntos
integrados não são reapresentadas nesta seção.

\subsection{Implementação do ambiente de consulta}

A versão avaliada reuniu uma interface desenvolvida em React e TypeScript, uma
API interna baseada em Express e um banco SQLite destinado às operações da
aplicação. Esses elementos foram distribuídos em uma configuração
conteinerizada, que reuniu o servidor, a interface compilada e a representação
dos dados usada em tempo de execução.

O banco SQLite apresentou as 12 tabelas requeridas pelo servidor e ocupou
15,77~MiB. A aplicação abriu esse arquivo em modo somente leitura, e as
consultas não exigiram o carregamento dos arquivos CSV e XLSX publicados. O
SQLite funcionou, portanto, como representação operacional do conteúdo
descrito no Capítulo~\ref{cap:comp1}, sem substituir os artefatos científicos
usados para distribuição e reutilização dos dados.

A API interna disponibilizou operações para os domínios de compostos, classes
químicas, genes e KOs, vias, toxicidade, metadados e análises guiadas. As
requisições realizadas durante a avaliação retornaram respostas válidas para
os módulos examinados e preservaram a paginação, a filtragem e a recuperação
das entidades relacionadas.

\subsection{Módulos de exploração e navegação entre entidades}

Foram disponibilizados os cinco módulos previstos: \textit{Compounds},
\textit{Compound Classes}, \textit{Genes / KO}, \textit{Pathways} e
\textit{Toxicity}. As respectivas interfaces são apresentadas nas
Figuras~\ref{fig:c2-compounds-explorer} a
\ref{fig:c2-toxicity-explorer}, enquanto suas unidades de consulta e relações
recuperadas são sintetizadas na Tabela~\ref{tab:c2-explorer-modules}.

O módulo de compostos permitiu recuperar as anotações funcionais, as vias, as
referências institucionais e as predições toxicológicas associadas a cada CPD.
O módulo de classes químicas agrupou os compostos pelo atributo empregado na
interface. Os módulos de genes e KOs e de vias permitiram iniciar a consulta
por entidades funcionais, enquanto o módulo de toxicidade organizou os
resultados por combinação entre composto e endpoint.

A navegação entre listagens e páginas de detalhe foi verificada a partir das
relações retornadas pela aplicação. Os registros de compostos conduziram aos
KOs, às vias e às predições toxicológicas associadas. Em sentido inverso, as
consultas iniciadas por KO ou via recuperaram os compostos correspondentes.
Também foi preservado o acesso ao registro do composto a partir dos módulos de
classes químicas e toxicidade.

Essa verificação examinou a correspondência entre as entidades apresentadas
nas listagens e aquelas recuperadas nas páginas de detalhe. Seu escopo não
incluiu avaliação de usabilidade com participantes ou mensuração do
desempenho da interface sob carga concorrente.
...
As oito análises usaram a estrutura compartilhada apresentada na
Figura~\ref{fig:c2-guided-structure}. As configurações YAML foram processadas
para definir os parâmetros, as medidas resumidas, as visualizações e as
tabelas de cada caso de uso. A execução pela API retornou essas informações
sob um formato comum, independentemente da entidade adotada como ponto de
entrada.

A interface resultante é exemplificada na
Figura~\ref{fig:c2-guided-analysis}. A mesma organização foi mantida entre os
casos de uso, com seleção da análise, definição dos parâmetros, apresentação
das medidas resumidas, visualização e tabela de resultados.

Cada caso de uso permaneceu associado a uma receita SQL versionada. As oito
receitas foram executadas sobre a mesma versão do banco SQLite e reproduziram
as unidades de análise, os filtros, as agregações, as métricas principais e os
critérios de ordenação definidos pela aplicação. Diferenças relacionadas à
paginação ou à seleção dos campos apresentados permaneceram restritas à
projeção da resposta e não modificaram as operações analíticas.

A combinação entre especificações YAML e receitas SQL permitiu registrar tanto
a configuração usada pela interface quanto a consulta executada sobre o banco.
Desse modo, os procedimentos analíticos puderam ser repetidos
independentemente da visualização disponibilizada pelo Database Explorer.
...
As verificações demonstraram o funcionamento da aplicação no escopo definido
para o componente. Os resultados não constituíram uma avaliação de desempenho
sob carga, de disponibilidade contínua do serviço ou de usabilidade com
participantes, aspectos que exigiriam protocolos específicos.
...
disponível em
\url{https://biorempp-dbx.readthedocs.io/en/stable/}. A versão avaliada da
aplicação corresponde à versão beta~1.0.0 e utiliza a versão~1.1.0 dos dados
BioRemPP.

O código foi distribuído sob a licença Apache~2.0, enquanto os dados
permaneceram disponíveis sob a licença CC~BY~4.0. A aplicação web e a
documentação ampliaram as formas de acesso; o uso de CPD e KO manteve a
interoperabilidade com identificadores externos; e as configurações, receitas
SQL, licenças e artefatos versionados forneceram mecanismos para reprodução e
reutilização. Esses elementos contribuíram para a implementação dos
princípios FAIR no escopo do componente.
...
A separação entre os arquivos publicados e o SQLite mantém funções distintas
...
interface não se torna, portanto, a única forma de acesso ao banco BioRemPP.

A API interna restringe o acoplamento entre a interface e a organização física
...
descrita no modelo do banco.

A navegação entre listagens e páginas de detalhe mantém o contexto das
...
A classe química exerce uma função distinta das relações estabelecidas por CPD
e KO. O valor apresentado na interface permite agrupar e filtrar compostos,
mas não participa da definição das associações funcionais, toxicológicas ou
institucionais. Essa delimitação impede que a classificação usada na
apresentação seja interpretada como uma nova relação biológica produzida pelo
Database Explorer.
...
de filtros e mantém explícito o procedimento aplicado a cada consulta.

A implementação declarativa em YAML associa a pergunta analítica aos
...
mudanças no procedimento analítico de modificações exclusivamente visuais.
...
e apresentado no Database Explorer.

A reprodução das métricas principais pelas receitas SQL indica
...
interface for comparada diretamente à consulta completa.

Os casos de uso mantêm caráter exploratório. Os rankings baseados em KOs,
...
também não substituem ensaios experimentais ou avaliações formais de perigo.

Os limiares calculados a partir do conjunto filtrado produzem classificações
...
A arquitetura atual é compatível com um banco versionado, predominantemente
consultivo e distribuído em arquivo único. Cenários com escrita concorrente,
atualizações frequentes ou aumento substancial da carga podem exigir outro
sistema de gerenciamento de dados ou separação adicional dos serviços. Essa
possibilidade constitui uma condição de evolução da infraestrutura e não
altera o conteúdo científico representado pelo componente.

As verificações técnicas avaliaram o comportamento da implementação no escopo
dos testes automatizados, das consultas à API e da execução conteinerizada.
Esses procedimentos não correspondem a ensaios de usabilidade, acessibilidade
com participantes ou desempenho sob carga concorrente. Avaliações dessa
natureza exigem protocolos próprios e podem orientar versões posteriores da
aplicação.
...
reutilização. Esses elementos não constituem, isoladamente, uma avaliação
formal de conformidade com todas as dimensões FAIR.
